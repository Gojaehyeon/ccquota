...
`console.anthropic.com/v1/oauth/token`의 429는 usage 조회의 429와 성격이 다릅니다.
**잘못된 토큰으로 요청해도 429가 나옵니다** — 제한이 자격증명 검증보다 앞단에 있다는 뜻이고,
계정 단위가 아니라 호출자(IP·클라이언트) 단위로 걸립니다. 관측된 지속 시간은 하루에 가깝습니다.

그리고 이 엔드포인트는 **`claude` 본체가 로그인 토큰을 갱신하는 곳과 같습니다.**
여기를 자주 두드리면 사용자의 Claude Code 로그인 자체가 갱신에 실패할 수 있습니다.
그래서 이 경우의 백오프는 30분에서 시작해 최대 6시간까지 물러나며, usage 쪽(5분~30분)보다 훨씬 깁니다.

제한이 풀렸는지는 실제 토큰을 쓰지 않고 확인할 수 있습니다.

```bash
ccquota probe    # 요청 1회. 무효 토큰을 보내므로 저장된 자격증명을 소모하지 않습니다
```
